...
public class LinkedListSummary {
	...
	public static Node reGetKthNode(Node head, int k) {
		...
		Node fast = head; // fast在slow前面 fast--slow
		Node slow = head; // slow在fast后面
		...
		while (fast.next != null) { // if fast != null, then last node is "null"
			fast = fast.next;
			slow = slow.next;
		}
	...
	public static void reGetKthNodeRec(Node head, int k) {
		...
		// I think this part of logic is not correct, comment out below
		// if(k == 1){
		// return;
		// }
	...
	/**
	 * 从尾到头打印单链表 对于这种颠倒顺序的问题，我们应该就会想到栈，后进先出。所以，这一题要么自己使用栈，要么让系统使用栈，也就是递归
	 * (系统使用栈=递归)。注意链表为空的情况 。时间复杂度为O（n）
	 */
